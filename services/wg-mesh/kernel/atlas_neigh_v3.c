...
__bpf_kfunc int atlas_register_neigh_v3(
	__u32 ifindex,
	__u64 addr_hi,
	__u64 addr_lo,
	__u64 mac)
{
	...
	if (!neighbour) {
		...
		if (IS_ERR(neighbour)) {
			...
			pr_info(
				"atlas_v3: __neigh_create failed: %d\n",
				result);
		...
		pr_info("atlas_v3: created neighbour\n");
	} else {
		pr_info(
			"atlas_v3: found neighbour state=0x%x flags=0x%x\n",
			neighbour->nud_state,
			neighbour->flags);
	}
	...
	if (result) {
		pr_info(
			"atlas_v3: lladdr update failed: %d\n",
			result);
	...
	pr_info(
		"atlas_v3: lladdr update state=0x%x flags=0x%x\n",
		neighbour->nud_state,
		neighbour->flags);
	...
	pr_info(
		"atlas_v3: managed update result=%d state=0x%x flags=0x%x\n",
		result,
		neighbour->nud_state,
		neighbour->flags);
...
static const struct btf_kfunc_id_set atlas_neigh_v3_kfunc_set = {
	.owner = THIS_MODULE,
	.set = &atlas_neigh_v3_kfunc_ids,
};

static int __init atlas_neigh_v3_init(void)
{
	int result;
...
module_exit(atlas_neigh_v3_exit);
