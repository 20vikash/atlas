...
func TestAdvanceTargetRunsTheHandshakeAndHides(t *testing.T) {
	...
	if record.Phase != PhaseCopying || record.Config == nil || record.UserID == 0 {
		t.Fatalf("record = %+v", record)
	}

	desired, err := machines.store.readDesired("vm-1")
	if err != nil {
	...
	if _, err := machines.Information(ctx, "vm-1"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("get of a target = %v, want ErrNotFound", err)
	}
	...
	if len(list) != 0 {
		t.Fatalf("target is visible in the list: %+v", list)
	}
	if err := machines.Reconcile(ctx, "vm-1"); err != nil {
		t.Fatalf("reconcile of a target = %v, want nil", err)
	}
}
...
func TestAdvanceTargetRejectsInsufficientCapacity(t *testing.T) {
	...
	migrationManager, err := NewMigrationManager(machines, source, &fakeTransfer{}, noCapacity, MigrationSettings{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	ctx := context.Background()
	...
	if err := migrationManager.AdvanceTarget(ctx, "vm-1"); !errors.Is(err, ErrConflict) {
