...
Use the numbered sections as detection rules during a code review. Each section gives a pattern to find, an example, and a preferred code shape.

Do not request a change only because a pattern matches. First make sure that the complexity does not represent a real state, boundary, or operational need.

For each finding, state the code location, the unnecessary complexity, and the simplest safe change. Keep valid boundary validation, error handling, resource cleanup, and synchronization.
...
# 74. Use review questions for each finding
...
### Step 2
Check that the input has a concrete type.

### Step 3
Check that the code validates and decodes the input once.
...
### Step 5
Find forwarding layers that have no behavior.

### Step 6
Find unused abstractions.

### Step 7
Request the relevant tests and static analysis.
...
This is a complexity review. Do not request reckless deletion.

---

# 78. Expected review outcome
