...
VALID_CONFIG = {
	...
	"storage": {"cpu": 4, "ram_gb": 8, "disk_gb": 500},
}
...
class TestStorageClusterConfig(UnitTestCase):
	...
	def test_a_node_size_needs_every_field(self) -> None:
		with self.assertRaisesRegex(frappe.ValidationError, "storage must hold cpu, ram_gb and disk_gb"):
			store_storage_cluster_config(VALID_CONFIG | {"storage": {"cpu": 4, "ram_gb": 8}})
	...
	def test_a_node_size_below_one_is_rejected(self) -> None:
		with self.assertRaisesRegex(frappe.ValidationError, "gateway disk_gb must be a whole number"):
			store_storage_cluster_config(VALID_CONFIG | {"gateway": {"cpu": 2, "ram_gb": 4, "disk_gb": 0}})
