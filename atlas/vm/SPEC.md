...
`balanced` prefers the current host for a start or resize when the request fits, then uses the Default host ranking. It expands at 77.5% aggregate memory or storage pressure. A wake already checks its current host before the strategy runs, so this policy reduces migrations without changing that wake check.
