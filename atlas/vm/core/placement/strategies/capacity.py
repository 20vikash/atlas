...
from atlas.vm.core.placement.models import HostUsage, PlacementRequest


def provisioning(host: HostUsage, request: PlacementRequest | None = None) -> float:
...
def below_provisioning_limit(host: HostUsage, percent: int, request: PlacementRequest | None = None) -> bool:
...
def pool_at_provisioning_limit(
	hosts: Sequence[HostUsage], percent: int, request: PlacementRequest | None = None
) -> bool:
...
def sleepy_best_fit(host: HostUsage, request: PlacementRequest) -> tuple[float, float, str]:
