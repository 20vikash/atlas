...
class TestArtifacts(UnitTestCase):
	...
	def test_download_url_refuses_a_missing_file(self) -> None:
		"""A deleted File must fail here, not as `...shopNone` inside the guest."""
		with (
			patch.object(artifacts.frappe.db, "get_value", return_value=None),
			patch.object(artifacts.frappe, "conf", SimpleNamespace(atlas_base_url="https://atlas.test/")),
		):
			with self.assertRaisesRegex(frappe.ValidationError, "0a77b2012b"):
				artifacts.get_download_url("0a77b2012b")
