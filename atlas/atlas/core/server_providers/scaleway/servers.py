...
class ScalewayServers:
	...
	def ensure(self, request: ServerCreateRequest) -> ProviderServer:
		...
		remote_server = self.find(request.identity_key)
	...
	def create(self, request: ServerCreateRequest) -> Mapping:
		...
		return self.client.request(
			...
			json={
				...
				"tags": [self.server_tag(request.identity_key)],
	...
	def find(self, identity_key: str) -> Mapping | None:
		...
		response = self.client.request(
			...
			params={
				"project_id": self.configuration.project_id,
				"tags": [self.server_tag(identity_key)],
			},
		)
		...
		if len(servers) > 1:
			raise ScalewayError(f"Scaleway returned multiple servers for identity key {identity_key}")
	...
	@staticmethod
	def server_tag(identity_key: str) -> str:
		"""Return the stable Atlas identity tag for one provider server."""
		return f"atlas-server:{identity_key}"
