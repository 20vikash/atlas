...
3. Set a unique provider identity key and the architecture from Metal Server Size.
...
1. Create or reuse the provider host by its stored identity key. The key is unique to the Metal Server record and stays the same on retry. The provider host name is a label; `provider_server_id` is the provider's host identifier.
