...
`handlers.py` validates the console token and owns the WebSocket bridge lifecycle. `realtime/handlers.js` in the repository root is the same bridge for the node socketio backend. Pilot uses the node backend by default.
...
The bridge connects to Metal with secure WebSocket transport. It loads the regional authority and the Atlas client pair from the private site directory, so the stored connection carries only the URL.
