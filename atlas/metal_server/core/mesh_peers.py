...
def get_mesh_peer_file_contents() -> str:
	...
	Every running Metal Server with a private IPv4 address becomes one peer line, in ascending address order. The unicast daemon drops the local address when it loads the file, so the same contents suit every host.
	"""
	rows = frappe.get_all(
		"Metal Server",
	...
	return "\n".join(sorted(peers, key=ipaddress.IPv4Address)) + "\n"
